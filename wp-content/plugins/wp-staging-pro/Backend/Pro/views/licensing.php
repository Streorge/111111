    ...
    <label for="wpstg_license_key" style='display:block;margin-bottom: 5px;margin-top:10px;'><?php echo sprintf(__('Enter your license key to activate WP STAGING | PRO. %s You can buy a license key on %s.', 'wp-staging'), '<br>', '<a href="https://wp-staging.com?utm_source=wpstg-license-ui&utm_medium=website&utm_campaign=enter-license-key&utm_id=purchase-key&utm_content=wpstaging" target="_blank">wp-staging.com</a>'); ?></label>
      ...
      <?php
        ...
        if (isset($license->error) && $license->error === 'expired') {
            ...
            $message .= '<p><a href="' . admin_url() . 'admin.php?page=wpstg_clone" id="wpstg-new-clone" class="wpstg-next-step-link wpstg-link-btn button-primary">' . __("Start using WP STAGING", "wp-staging") . '</a>';
        } else {
            $message = '';
        }
        ...
        if (isset($license->license) && $license->license === 'valid') {
            ...
            echo '<input type="submit" class="wpstg-button wpstg-blue-primary" value="' . __('Activate License', 'wp-staging') . '">';
        }
        ?>
        </form>
        <?php echo '<div style="padding:3px;">' . $message . '</div>'; ?>
</div>
