...
if ( ! class_exists( 'Farmart_Widget_Product_Categories' ) ) {
	...
	class Farmart_Widget_Product_Categories extends WC_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'A list of product categories.', 'farmart' );
			$this->widget_id          = 'fm_product_categories';
			$this->widget_name        = esc_html__( 'Farmart - Product Categories', 'farmart' );
			$this->settings           = array(
				'title'                     => array(
					'type'  => 'text',
					'std'   => esc_html__( 'Product categories', 'farmart' ),
				...
				'orderby'                   => array(
					...
					'options' => array(
						...
						'title' => esc_html__( 'Name', 'farmart' ),
					),
				),
				'show_dropdown'                     => array(
					'type'  => 'checkbox',
					'std'   => 0,
					'label' => esc_html__( 'Show dropdown widget', 'farmart' ),
				),
				'count'                     => array(
					'type'  => 'checkbox',
					'std'   => 0,
					'label' => esc_html__( 'Show product counts', 'farmart' ),
				),
				'hide_empty'                => array(
					'type'  => 'checkbox',
					'std'   => 0,
					'label' => esc_html__( 'Hide empty categories', 'farmart' ),
				),
				'show_all_cats'             => array(
					'type'  => 'checkbox',
					'std'   => 1,
					'label' => esc_html__( 'Only show children of the current category', 'farmart' ),
					'class' => 'fm_categories_show_children_only'
				),
				'show_first_level_children' => array(
					'type'  => 'checkbox',
					'std'   => 0,
					'label' => esc_html__( 'Only show the first level children of the current category', 'farmart' ),
					'class' => 'fm_categories_show_children_only_els'
				),
				'max_depth'                 => array(
					'type'  => 'text',
					'std'   => '',
					'label' => esc_html__( 'Maximum depth', 'farmart' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			if ( $this->current_cat && $show_children_only ) {
				...
				$back_shop     = sprintf( '<li class="fm-back-shop"><a href="%s">%s %s</a></li>', esc_url( get_post_type_archive_link( 'product' ) ), esc_html__( 'All', 'farmart' ), $count_html );
