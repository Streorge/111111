...
class Socials_Walker extends \Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( $item_icon_type === 'svg' ) {
			$item_icon_color = $item_icon_color ? 'style="fill:' . $item_icon_color . '; color:' . $item_icon_color . ';"' : '';
			$item_svg_html  = ! empty( $item_icon_svg ) ? '<span ' . $item_icon_color . ' class="farmart-svg-icon">' . \Farmart\Icon::sanitize_svg( $item_icon_svg ) . '</span> ' : '';
		}elseif ( $item_icon_type === 'text' ) {
			$item_svg_html  = ! empty( $item_icon_text ) ? '<span class="farmart-text-icon">' . wp_kses_post( $item_icon_text ) . '</span> ' : '';
