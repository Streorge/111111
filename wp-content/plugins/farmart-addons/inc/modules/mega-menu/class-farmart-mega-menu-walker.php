...
class Walker extends \Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$item_is_mega         = apply_filters( 'farmart_menu_item_mega', get_post_meta( $item->ID, 'tamm_menu_item_mega', true ), $item->ID );
		...
		// Container
		$class = array( 'mega-menu-content row-mega' );
		...
		if ( $item_icon_type === 'svg' ) {
			...
			$item_icon_html  = ! empty( $item_icon_svg ) ? '<span ' . $item_icon_color . ' class="farmart-svg-icon">' . \Farmart\Icon::sanitize_svg( $item_icon_svg ) . '</span> ' : '';
		}elseif ( $item_icon_type === 'text' ) {
			$item_icon_text_color = $item_icon_text_color ? 'style="color:' . $item_icon_text_color . '"' : '';
			$item_icon_html  = ! empty( $item_icon_text ) ? '<span ' . $item_icon_text_color . ' class="farmart-text-icon">' . wp_kses_post( $item_icon_text ) . '</span> ' : '';
		...
		if ( $depth == 1 && intval( $this->in_mega ) ) {
			...
			$class_names = ' class="menu-item-mega col-mega col-mega-' . $this->column . '"';
		...
		if ( $depth == 1 && intval( $this->in_mega ) ) {
			...
			$item_output = '<a ' . $attributes . '>' . $icon_output . $title . '</a>';
