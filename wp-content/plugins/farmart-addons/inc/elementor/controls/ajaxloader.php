	...
	public function fm_autocomplete_taxonomy_callback( $taxonomy = 'category' ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = esc_html__( 'Id', 'farmart' ) . ': ' . $value['id'] . ' - ' . esc_html__( 'Name', 'farmart' ) . ': ' . $value['name'];
				$result[]      = $data;
			}
		} else {
			$result[] = array(
				'value' => 'nothing-found',
				'label' => esc_html__( 'Nothing Found', 'farmart' )
			);
		}
	...
	public function fm_autocomplete_post_type_callback( $post_type = 'product' ) {
		...
		if ( is_array( $posts ) && ! empty( $posts ) ) {
			foreach ( $posts as $post ) {
				$data          = array();
				$data['value'] = $post->ID;
				$data['label'] = esc_html__( 'Id', 'farmart' ) . ': ' . $post->ID . ' - ' . esc_html__( 'Title', 'farmart' ) . ': ' . $post->post_title;
				$result[]      = $data;
			}
		} else {
			$result[] = array(
	...
	public function fm_autocomplete_taxonomy_render( $taxonomy = 'category' ) {
		...
		foreach ( $terms as $term ) {
			...
			$data[] = sprintf(
				...
				esc_attr( $term->slug ),
				esc_html__( 'Id: ', 'farmart' ),
				esc_html( $term->term_id ),
				esc_html__( 'Name: ', 'farmart' ),
				esc_html( $term->name )
			);
		}
