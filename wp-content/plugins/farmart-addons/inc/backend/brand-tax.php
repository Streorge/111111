...
class Farmart_Product_Brands {
	...
	public function register_brand() {
		...
		$labels = array(
			'name'                       => __( 'Brands', 'farmart' ),
			'singular_name'              => __( 'Brand', 'farmart' ),
			'menu_name'                  => __( 'Brands', 'farmart' ),
			'all_items'                  => __( 'All Brands', 'farmart' ),
			'edit_item'                  => __( 'Edit Brand', 'farmart' ),
			'view_item'                  => __( 'View Brand', 'farmart' ),
			'update_item'                => __( 'Update Brand', 'farmart' ),
			'add_new_item'               => __( 'Add New Brand', 'farmart' ),
			'new_item_name'              => __( 'New Brand Name', 'farmart' ),
			'parent_item'                => __( 'Parent Brand', 'farmart' ),
			'parent_item_colon'          => __( 'Parent Brand:', 'farmart' ),
			'search_items'               => __( 'Search Brands', 'farmart' ),
			'popular_items'              => __( 'Popular Brands', 'farmart' ),
			'separate_items_with_commas' => __( 'Separate brands with commas', 'farmart' ),
			'add_or_remove_items'        => __( 'Add or remove brands', 'farmart' ),
			'choose_from_most_used'      => __( 'Choose from the most used brands', 'farmart' ),
			'not_found'                  => __( 'No brands found', 'farmart' )
		);

		$permalinks         = get_option( 'product_brand_permalinks' );
		$product_brand_base = empty( $permalinks['product_brand_base'] ) ? _x( 'product-brand', 'slug', 'farmart' ) : $permalinks['product_brand_base'];
	...
	public function settings_api_init() {
		add_settings_section(
			...
			'<span id="brand-options">' . esc_html__( 'Product brand', 'farmart' ) . '</span>',
		...
		add_settings_field(
			$this->option,
			'<span class="brand-options">' . esc_html__( 'Product brand', 'farmart' ) . '</span>',
		...
		add_settings_field(
			'product_brand_slug',
			'<label for="product_brand_slug">' . esc_html__( 'Product brand base', 'farmart' ) . '</label>',
	...
	public function product_brand_slug_input() {
		...
		?>
		<input name="product_brand_slug" type="text" class="regular-text code" value="<?php echo esc_attr( $brand_base ); ?>" placeholder="<?php echo esc_attr_x( 'product-brand', 'slug', 'farmart' ) ?>" />
		<?php
	}
	...
	public function product_brand_column( $columns, $column, $id ) {
		...
		if ( 'thumb' == $column ) {
			...
			$columns .= '<img src="' . esc_url( $image ) . '" alt="' . esc_attr__( 'Thumbnail', 'farmart' ) . '" class="wp-post-image" height="48" width="120" />';
	...
	public function disable_field_html() {
		...
		<label for="<?php echo esc_attr( $this->option ); ?>">
			...
			<?php esc_html_e( 'Disable Brand for this site.', 'farmart' ); ?>
		</label>
