			...
			if ( isset( $_POST['mwb_wgm_offline_gift_template'] ) && $_POST['mwb_wgm_offline_gift_template'] != null ) {
				...
				if ( $send_mail ) {
					...
					<div class="updated notice notice-success is-dismissible" id="message">
						<p>
							<?php _e( 'Giftcard Created and Mail is sent to Sender and Receiver. Code is', 'woocommerce-ultimate-gift-card' ); ?> : <a href="javascript:void(0);"><?php echo $gift_couponnumber; ?></a>
						</p>
					</div>
					<?php
				} else {
					...
					<tbody>
						...
						<tr valign="top">
							...
							<td class="forminp forminp-text">
								<?php
									$attribute_description = __( 'Select the scheduled date for sending the gift card on the specified date.', 'woocommerce-ultimate-gift-card' );
		...
		<tbody>
			<tr valign="top">
				<td colspan="3" class="mwb_wgm_instructions_tabledata">	
					...
					<p> 1- <?php _e( 'It just provides you the way from where you can import your coupons in bulk and can provide them Manually to your Customers. You need to choose a CSV file and click Import', 'woocommerce-ultimate-gift-card' ); ?></p>
					<p>2- <?php _e( 'CSV for Offline Coupons must have 4 columns in this order ( Coupon Code, Expiry Date, Usage Limit, Price. Also the first row must be the respective headings. )', 'woocommerce-ultimate-gift-card' ); ?> </p>
		...
		<table class="form-table">
			<tr valign="top">
				<td class="forminput">
					...
					<p>2- <?php _e( 'CSV for Offline Gift Card must have 5 columns in this order ( To, From, Message, Price, Schedule Date. Also the first row must be the respective headings. )', 'woocommerce-ultimate-gift-card' ); ?> </p>
