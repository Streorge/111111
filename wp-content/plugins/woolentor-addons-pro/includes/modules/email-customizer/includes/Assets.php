...
class Assets {
    ...
    public function frontend_enqueue_scripts() {
        if ( woolentor_is_email_customizer_template() ) {
            ...
            $generated_css_url = $wp_upload_dir['baseurl'] . '/elementor/css';
            $generated_css_path = $wp_upload_dir['basedir'] . '/elementor/css';

            $template_css_url = $generated_css_url . '/post-' . $post_id . '.css';
            $template_css_path = realpath( $generated_css_path . '/post-' . $post_id . '.css' );

            wp_enqueue_style( 'woolentor-email-elementor-frontend', WOOLENTOR_EMAIL_CUSTOMIZER_ASSETS . '/elementor/css/frontend-lite.min.css', array(), '3.6.5' );
