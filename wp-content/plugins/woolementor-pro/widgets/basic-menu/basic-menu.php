...
class Basic_Menu extends Widget_Base {
	...
	public function __construct( $data = [], $args = null ) {
		...
		wp_register_script( "woolementor-{$this->id}-smartmenus", plugins_url( "assets/lib/smartmenus/jquery.smartmenus{$min}.js", __FILE__ ), ['jquery'], '1.1', true );
		wp_register_script( "woolementor-{$this->id}", plugins_url( "assets/js/script{$min}.js", __FILE__ ), ['jquery'], '1.1', true );

		wp_register_style( "woolementor-{$this->id}-sm-blue", plugins_url( "assets/lib/smartmenus/sm-blue.min.css", __FILE__ ), [], '1.1' );
	...
	protected function render() {
		...
		echo "<div class='woolementor-basic-menu-wrapper ". esc_attr( $class ) ."'>";

		$args = [
			'echo' => false,
			'menu' => $settings['menu'],
			'menu_class' => 'elementor-nav-menu woolementor-nav-menu',
		...
		$this->add_render_attribute( 'menu-toggle', [
			'class' => 'elementor-menu-toggle woolementor-menu-toggle',
			'role' => 'button',
			'tabindex' => '0',
			'aria-label' => __( 'Menu Toggle', 'woolementor-pro' ),
			'aria-expanded' => 'false',
		] );
		...
		if ( 'dropdown' !== $settings['layout'] ) :
			$this->add_render_attribute( 'main-menu', 'class', [
				'elementor-nav-menu--main woolementor-nav-menu--main',
				'woolementor-nav-menu__container woolementor-nav-menu__container',
			...
			<nav <?php echo $this->get_render_attribute_string( 'main-menu' ); ?>><?php echo wp_kses_post( $menu_html ); ?></nav>
			<?php
		endif;
		?>
		<div <?php echo $this->get_render_attribute_string( 'menu-toggle' ); ?>>
			<i class="eicon-menu-bar" aria-hidden="true"></i>
			<span class="woolementor-screen-only"></span>
		</div>
			<nav class="woolementor-nav-menu--dropdown woolementor-nav-menu__container" role="navigation" aria-hidden="true"><?php echo wp_kses_post( $dropdown_menu_html ); ?></nav>
		<?php
	...
	public function handle_link_classes( $atts, $item, $args, $depth ) {
		$classes = $depth ? 'woolementor-sub-item' : 'elementor-item woolementor-item';
		$is_anchor = false !== strpos( $atts['href'], '#' );

		if ( ! $is_anchor && in_array( 'current-menu-item', $item->classes ) ) {
			$classes .= ' elementor-item-active woolementor-item-active';
		}

		if ( $is_anchor ) {
			$classes .= ' woolementor-item-anchor';
		}
	...
	public function handle_sub_menu_classes( $classes ) {
		$classes[] = 'woolementor-nav-menu--dropdown';
